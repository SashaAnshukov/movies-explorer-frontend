...
function App() {
  ...
  const [favoriteShortCards, setFavoriteShortCards] = useState(false); // стэйт короткометражек в saved-movies (чекбокса)
  ...
  const deleteFavoriteMovie = (card) => {
      mainApi.deleteMovie(card._id)
        .then(() => {
            ...
            const newFilteredFavouriteList = filteredFavoriteCards.filter(
              (favoriteCards) => favoriteCards._id !== card._id
          );
            updateFavoriteMovies(newFavouriteList)
            updateFilteredFavoriteMovies(newFilteredFavouriteList)
        })
        .catch(err => {
            console.log (`Ошибка: ${err}`)
        })
  ...
  useEffect(() => {
    ...
    if(localFilteredFavoriteCards && localFilteredFavoriteCards.length){
      try {
        ...
        localStorage.removeItem('all_filtered_favorite_movies', JSON.stringify(favoriteCards));
      }
      catch (err){
  ...
  const updateFavoriteShortMovies = (favoriteShortCards) => {
    setFavoriteShortCards(favoriteShortCards);
    localStorage.setItem('all_favorite_short_movies', JSON.stringify(favoriteShortCards));
  }
  ...
  useEffect(() => {
    ...
    updateFavoriteShortMovies(
      JSON.parse(localStorage.getItem('all_favorite_short_movies') || 'false')
    );

    setFilteredFavoriteCards(favoriteCards)
    setFavoriteQuery('')
    setFavoriteShortCards(false)

  }, [navigate])


  //фильтрация результата поиска короткометражек и вывод карточек на страницу
  const searchResult = filteredCards.filter((card) => !shortCards || card.duration <= 40)
  const  searchFavoriteResult = favoriteCards.filter((card) => !favoriteShortCards || card.duration <= 40)
  const searchFilteredFavoriteResult = filteredFavoriteCards.filter((card) => !favoriteShortCards || card.duration <= 40)
          ...
          <Route exact path='/saved-movies' element = {
            <ProtectedRoute loggedIn={loggedIn} >
              <Movies 
                ...
                query={favoriteQuery}
                shortCards={favoriteShortCards}
                updateQuery={updateFavoriteQuery}
                updateShortMovies={updateFavoriteShortMovies}
                isLikedCard={isLikedCard}
                onCardLike={onCardLike}
